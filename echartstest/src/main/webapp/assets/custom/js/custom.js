...
function areaPosition(){
	var outWidth = $(this).width();
    	if(outWidth>1000){
    		...
    		$(".ui-co-area").css("right",30);
    	}  
}

function polygonLocation(uiId, contextPath, uiDisplayId, dialogId, viewOnly) {
	$("#" + uiId).click(function () {
		quick_dialog(uiId, "定位", contextPath + "/gis/getPolygon?polygonLocation=" + $("#" + uiDisplayId).val()
			+ "&uiDisplay=" + uiDisplayId + "&dialogId=" + dialogId + "&viewOnly=" + viewOnly, 1000, 600);
	});
}