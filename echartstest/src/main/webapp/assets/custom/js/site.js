...
function action_manage_confirm(title, message, callback, btn) {
    ...
    quick_dialog("action_manage_confirm_dlg", title, {
        template: "<div class=\"ui-dialog-form\" style='padding: 10px;'> <div class=\"k-error-colored\" style='padding: 20px;text-align: left;font-size: 12px;'> <i class=\"fa fa-exclamation-circle\" style='font-size: 45px;padding: 20px 20px 0 10px;display: inline-block;float: left;'></i> <div class=\"fn-left\"> <p>" + message + "</p> <p>再次确认请输入</p> <table style=\"margin-top: 10px;\"><tr><td width=\"80\" align=\"left\" class=\"label\">管理员密码：</td><td width=\"200\"><input name=\"password\" class=\"k-textbox\" type=\"password\"></td><td></td></tr></table></div><div class=\"fn-clear\"></div></div>" +
...
function get_el_in_main_frame(el) {
    ...
    var iframe = $("#_main_frame");
    if (!iframe)return null;
    if(!iframe[0].contentWindow)return null;
    try{
        return iframe[0].contentWindow.get_el(el);
    }catch(e){
        return null;
...
/* 获取父页面对象*/
function get_parent_el(el){
    if(!el)return null;
    return   $(el,window.parent["_main_frame"].document);
}
...
function quick_treelist(el, formId, readUrl, columns, options, dsOptions, operates, toolbarId) {
    ...
    if (operates && operates.length > 0) {
        $.each(operates, function (idx, it) {
            var iconClassArr = it.icon.split('-');
            var iconClass = iconClassArr[1];
            var $oper = $('<li class="'+ iconClass+'"><button class="k-button"' + (it.id ? 'id="' + it.id + '"' : '') + '>' + (it.icon ? '<i class="' + it.icon + '"></i>' : '') + it.name + '</button></li>');
...
function quick_datagrid(el, formId, readUrl, columns, options, dsOptions, operates, toolbarId) {
    ...
    if ($(toolbarId).html() != undefined) {
        toolbarHtml = hc.template($(toolbarId).html().replace(/&lt;/g, '<').replace(/&gt;/g, '>'));
    }
    var _opt = {
    ...
    if (operates && operates.length > 0) {
        $.each(operates, function (idx, it) {
            try{
            var iconClassArr = it.icon.split('fa-');
            var iconClass = iconClassArr[1];
            var $oper = $('<li class="'+ iconClass+'"><button ' + (it.id ? 'id="' + it.id + '"' : '') + '>' + (it.icon ? '<i class="' + it.icon + '"></i>' : '') + it.name + '</button></li>');
            if (it.onclick && $.isFunction(it.onclick)) {
                $("button", $oper).bind("click.toolbar", function (e) {
 ...
 * 参数(toolbarId)：工具条的id，可选
 **/
function quick_datagrid_custom(el, formId, readUrl, columns, options, dsOptions, operates, toolbarId) {
    options = options || {};
    toolbarId = toolbarId || "#grid_toolbar_template";
    var _dbopt = {
        transport: {
...
function quick_datagrids(el,formId,readUrl,columns,options,dsOptions,operates,toolbarId){
    ...
    if(operates && operates.length > 0){
        $.each(operates,function(idx,it){
            ...
            var iconClass = iconClassArr[1];
            //var $oper = $('<li class="'+ iconClass+'"><button class="k-button"' + (it.id ? 'id="' + it.id + '"' : '') + '>' + (it.icon ? '<i class="' + it.icon + '"></i>' : '') + it.name + '</button></li>');
            var $oper = $('<li class="'+ iconClass+'"><a href="javascript:void(0)" class="k-button">' + (it.icon ? '<i class="' + it.icon + '"></i>' : '') + it.name + '</a></li>');
            if(it.onclick && $.isFunction(it.onclick)){
...
function notify(message, status, autoHide) {
    ...
    if (typeof autoHide != 'boolean') {
        autoHide = (status == "success" || status == "warn" || status == "error") ? true : false;
    }
    if (window != top.window) {
...
//    $("._current_date_time").val(new Date().format("yyyy-MM-dd HH:mm:ss"));
    ...
    $(":reset").click(function(event){
        var selects =  $("form select").length;
        $("form :text").val('');
        $("form :text").attr("value",'');
        if(selects>0){
            $("form select").each(function(i,item){
                var combobox =$("form select").eq(i).data("kendoComboBox");
                combobox.select(1);
                combobox.select(0);
            })
        }
